...
export function buildScanPrompt(
  ...
  // GitHub scans may publish with the person's credential after confirmation.
  // Headless GitHub scans get no credential and remain report-only.
  const cs = repo.host === "codestorage";
  const reportOnly = !cs && githubPublication === "report-only";
  ...
  const findingUnit = cs ? "branches" : reportOnly ? "findings" : "PRs";
  const objective = cs
    ? "push one fix branch"
    : reportOnly
      ? "report"
      : "open one PR";
  const publishFinding = cs
    ? `For a confirmed finding, branch off ${branchLabel}: \`git checkout -b deepsec-scan-<short-slug> ${remoteRefArg}\`. Write a MINIMAL fix targeting the root cause only.
   - Validate that anything you change parses/builds where cheaply possible.
   - If you are NOT confident in a safe fix (e.g. it touches core auth broadly), do NOT guess — push the branch with the finding writeup + recommended fix in the commit message and NO code change, and add "[needs-human-fix]" to the commit title.
8. Commit with message "[deepsec][scan] <concise title>" plus a body covering finding, impact, fix, severity (note: generated by an on-demand ${productName()} security scan via vercel-labs/deepsec), then push the branch with \`git push -u origin deepsec-scan-<short-slug>\`. This repo is hosted on Code Storage; there is no gh CLI and no pull requests; a pushed branch IS the change request. One branch per finding. NEVER merge.`
    : reportOnly
      ? `For a confirmed finding, describe the smallest safe fix and how to validate it. Do not modify repository files or run GitHub write commands.
8. Do not commit, push, or open a pull request. Include the finding, impact, proposed fix, severity, and exact file and line in the final summary.`
      : `For a confirmed finding, branch off ${branchLabel}: \`git checkout -b deepsec-scan-<short-slug> ${remoteRefArg}\`. Write a MINIMAL fix targeting the root cause only.
   - Validate that anything you change parses/builds where cheaply possible.
...
8. Commit, push, then \`gh pr create --repo ${repo.ghRepo} --base ${branchArg} --title "[deepsec][scan] <concise title>" --body "<finding, impact, fix, severity; note: generated by an on-demand ${productName()} security scan via vercel-labs/deepsec>"\`. One PR per finding. NEVER run \`gh pr merge\`.`;
  const artifacts = cs
    ? "pushed branch names"
    : reportOnly
      ? "confirmed findings with exact file and line references"
      : "PR links";
  const publicationGuardrail = reportOnly
  ...
  return `You are ${personaName()} running an on-demand deepsec security scan on ${repo.id} (${repoLabel}). Sweep the repo for vulnerabilities and ${objective} per CONFIRMED finding of severity MEDIUM or higher. ${publicationGuardrail}
...
## Per finding (MEDIUM/HIGH/CRITICAL only; cap 8 ${findingUnit} per run — if more, list the overflow in the summary)
...
- Keep each ${reportOnly ? "finding" : cs ? "fix branch" : "PR"} small and focused; deduplicate if two findings share one root cause.${scanExtras(profile, instructions)}`;
}
  ...
  for (const repoId of scan.repos) {
    ...
    try {
      ...
      for await (const event of runAgent({
        ...
        // not host logins. Headless GitHub scans receive no GitHub credential;
        // they report findings without publishing repository changes.
        claudeCliEnv: true,
        codexCliEnv: true,
