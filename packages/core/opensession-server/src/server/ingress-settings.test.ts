...
describe("public ingress settings", () => {
  test("requires a separate public HTTPS origin", () => {
    ...
    expect(() =>
      normalizeIngressOrigin("https://ingress.example.test/opensession"),
  ...
  test("custom domains do not require URL syntax", () => {
    ...
    expect(
      normalizeCustomIngressOrigin("ingress.example.test/opensession/"),
    ).toBe("https://ingress.example.test/opensession");
    expect(() =>
      normalizeCustomIngressOrigin("ingress.example.test/open session"),
  ...
  test("persists a namespaced custom callback base but rejects one for a managed tunnel", async () => {
    ...
    expect(JSON.parse(readFileSync(path, "utf8")).ingress.publicBaseUrl).toBe(
      "https://hooks.example.test/opensession",
    );
    expect(readFileSync(envPath, "utf8")).toContain(
      "OPENSESSION_INGRESS_BASE=https://hooks.example.test/opensession",
    );
    await expect(
      savePublicIngress({
        publicBaseUrl: "https://hooks.example.test/opensession",
