...
async function attachSandbox(
  ...
  if (!provider)
    return Response.json(
      {
        error: "Name the Sandbox provider to move to: docker, daytona or box.",
      },
      { status: 400 },
    );
...
export async function handleSandboxRoutes(
  ...
  if (isRetiredSandboxProvider(recorded.provider))
    return Response.json(
      {
        error: `The ${recorded.provider} Sandbox provider has been retired; start a new session on Docker, Daytona or Box.`,
      },
      { status: 410 },
    );
  if (action === "desktop") {
