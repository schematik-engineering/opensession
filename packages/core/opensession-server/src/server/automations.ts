...
function sanitizeAccountId(
  ...
  if (
    !getAccountById(id) &&
    !getCodexAccountById(id) &&
    !getXaiAccountById(id) &&
    !getAcpAccountById(id)
  )
...
export function validateAutomationAccountPin(
  ...
  const accountProvider = getAccountById(automation.accountId)
    ...
    : getCodexAccountById(automation.accountId)
      ? "codex"
      : getXaiAccountById(automation.accountId)
        ? "xai"
...
export function validateSandboxAutomation(
  ...
  } else if (/^(grok|cursor)\//.test(runModel)) {
    const account = getAcpAccountById(automation.accountId);
    if (!account || !runModel.startsWith(`${account.provider}/`)) {
      return {
        error:
          "the pinned account does not belong to the selected SuperGrok or Cursor model",
      };
    }
  } else {
    return {
      error:
        "sandbox automations require an Anthropic, OpenAI, xAI, SuperGrok, or Cursor subscription model with a pinned account",
    };
  }
  ...
  try {
    ...
    if (sandbox) {
      ...
      events =
        ...
        providerFor(runModel) === "grok" ||
        providerFor(runModel) === "cursor"
          ? runAgentHosted({
              ...common,
              osSessionId: bksId,
