  ...
  return {
    async release() {
      ...
      if (!owner || owner.pid !== pid || owner.nonce !== nonce) {
        throw new Error(
          "Gateway activation lease ownership changed before release",
        );
      }
...
export async function waitForRuntimePeerGeneration(
  ...
  if (
    ...
    (executorDisabled ||
      !expectedExecutor ||
      expectedExecutor === "development")
  )
    return;
  for (const expected of [
    expectedKernel,
    ...(executorDisabled ? [] : [expectedExecutor]),
  ]) {
