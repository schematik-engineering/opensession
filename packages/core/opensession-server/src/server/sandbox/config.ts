...
/** Where a docker sandbox's workspace lives: "bind" (default) bind-mounts the
 *  existing host worktree; "volume" clones inside the container. */
export type SandboxWorkspaceMode = "bind" | "volume";

/** How a sandboxed run talks to Open Session. Remote providers always use ws. */
export type SandboxTransport = "socket" | "ws";

export const DEFAULT_SANDBOX_PREVIEW_PORTS = [3300, 3301, 3302] as const;

/** Snapshot-based warm restores for the docker provider. */
...
export interface SandboxConfig {
  ...
  /** Container image for the docker provider. */
  image?: string;
  ...
  cpus?: number;
  memory?: string;
  /** Workspace mode for NEW docker sandboxes. Default "bind". */
  workspace?: SandboxWorkspaceMode;
  /** Container ports to publish for previews. */
  previewPorts?: number[];
...
const PROVIDER_IDS = new Set<string>(["local", "docker", "daytona", "box"]);
...
export const RUNNABLE_SANDBOX_PROVIDERS = ["docker", "daytona", "box"] as const;
...
function sandboxProviderSelectionError(
  ...
  return `Sandbox provider "${id}" is not configured: ${
    id === "docker"
      ? "run opensession sandbox enable docker"
      : `connect ${id === "box" ? "Box" : "Daytona"} in Workspace > Sandboxes`
  }.`;
}
