...
const IMAGE_REFERENCE_SOURCE =
	String.raw`\[Image (\d+) · (\d+)–(\d+)% × (\d+)–(\d+)%\]`;
const IMAGE_REFERENCE_RE = new RegExp(`${IMAGE_REFERENCE_SOURCE}[ \\t]?`, "g");
const IMAGE_COMMENT_RE = new RegExp(`${IMAGE_REFERENCE_SOURCE}[ \\t]+([^\\n]+)`, "g");
...
export function parseImageAttachmentComments(
	...
	for (let match = IMAGE_COMMENT_RE.exec(text); match; match = IMAGE_COMMENT_RE.exec(text)) {
...
function findComment(
	text: string,
	existing: Pick<ImageAttachmentComment, "id" | "imageIndex" | "region" | "text">,
